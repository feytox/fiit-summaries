...
Д-во 1:
```spoiler-markdown
Если $\vec{a} || \vec{b} \Rightarrow \vec{a} \times \vec{b} = \vec{b} \times \vec{a} = \vec{0}$
Пусть $\vec{a} \cancel{ || } \vec{b}$. Так как $\sin(\angle(\vec{a}, \vec{b})) = \sin(\angle{(\vec{b}, \vec{a})})$:
...
$\vec{a} \times \vec{b}$ и $\vec{b} \times \vec{a}$ ортогональны $\vec{a}$ и $\vec{b}$, значит $\vec{a} \times \vec{b} || \vec{b} \times \vec{a}$.
