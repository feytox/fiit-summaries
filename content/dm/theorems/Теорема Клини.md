...
Если $L$ – регулярный, то $L$ распознается ДКА:
$$A = (Q, \Sigma, \delta, s, T),~~ L = L(A)$$
Рассмотрим автомат с инвертированными допускающими состояниями:
$$B = (Q, \Sigma, \delta, s, Q \setminus T)$$
$$w \in L(A) \iff w \notin L(B)$$
Следовательно $L(B) = \overline{L(A)} = \bar{L}$ и он является тоже регулярным, т.к. распознается ДКА.

Пусть $L_1, L_2$ – регулярные. Тогда:
