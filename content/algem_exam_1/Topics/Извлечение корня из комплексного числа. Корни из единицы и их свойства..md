### Определение: Извлечения корня из комплексного числа
```spoiler-markdown
...
Если $z = r(\cos\varphi + i \sin\varphi) \neq 0$, то корни $n$-ной степени $n$, они задаются формулой:
...
$$q^{n}(\cos n\psi+\sin n\psi)=r(\cos\varphi+i\sin\varphi)$$
