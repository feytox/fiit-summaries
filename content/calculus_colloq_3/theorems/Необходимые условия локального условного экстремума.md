...
Если $\mathbf{x}^0$ - точка условного локального экстремума функции $f(\mathbf{x})$, при наличии уравнений связи $g_k(\mathbf{x}) = 0, k = \overline{1, n}$, то $\exists{\lambda_1, \dots, \lambda_n \in \mathbb{R}}$ такие, что $\mathbf{x}^0$ - стационарная точка следующей функции:
$$
