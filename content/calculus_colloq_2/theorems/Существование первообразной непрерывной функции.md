...
Д-во:
```spoiler-markdown
Рассмотрим функцию $F(x) = \int\limits_{a}^{b} f(t) \, dt ~~\forall{x \in [a, b]}$ ([[Интеграл с переменным верхним пределом]])
Тогда запишем [[Дифференцируемость функции#Производная|определение производной]]:
...
$$\left| \dfrac{\int\limits_{x}^{x + \Delta x} f(t) \, dt }{\Delta x} - \dfrac{\int\limits_{x}^{x + \Delta x} f(x) \, dx }{\Delta x} \right| = \left| \dfrac{\int\limits_{x}^{x + \Delta x} (f(t) - f(x)) \, dt }{\Delta x} \right| ~~~~(*)$$
