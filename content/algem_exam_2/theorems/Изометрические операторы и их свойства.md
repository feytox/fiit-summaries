...
В комплексном случае рассмотрим следующее:
$$ |x + i y|^2 - |x - i y|^2 = 2i[(x, y) - (y, x)] $$
$$ |\mathcal{A} x + i\mathcal{A} y|^2 - |\mathcal{A} x - i\mathcal{A} y|^2 = 2i[(\mathcal{A} x, \mathcal{A} y) - (\mathcal{A} y, \mathcal{A} x)] $$
Отсюда:
...
$$(x, y - \mathcal{A}^{*}\mathcal{A}y) = 0 \overbrace{ \implies }^{ * } y(\mathcal{E} - \mathcal{A}^{*}\mathcal{A}) = 0 \overbrace{ \implies }^{ * } \mathcal{A}^{*}\mathcal{A} = \mathcal{E}$$
$*$ - так как $x$ и $y$ - любые.
...
## Теорема: Критерий унитарности оператора
Формулировка:
```spoiler-markdown
...
## Теорема: Структура изометрии в евклидовом пространстве
Формулировка:
```spoiler-markdown
