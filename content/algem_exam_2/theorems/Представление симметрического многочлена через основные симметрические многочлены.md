...
$$\operatorname{mdeg} g = \operatorname{mdeg} A \sigma_{1}^{s_{1}} \sigma_{2}^{s_{2}} \dots \sigma_{n}^{s_{n}} =^{*} \sum_{k=1}^{n}\sigma_{k}^{s_{k}} = (s_{1},0,\dots,0) + (s_{2}, s_{2}, 0, \dots, 0) + \dots + (s_{n}, s_{n}, \dots, s_{n}) \qquad (1)$$
$*$ - по лемме выше.
